...
allowed-tools: Bash(${CLAUDE_SKILL_DIR}/*) AskUserQuestion
...
2. **Generate the QR code.** Run the wrapper script from the user's current working directory:
   ```bash
   ${CLAUDE_SKILL_DIR}/run <text-or-url> [flags]
   ```
...
- PNG output is 200x200px with a 2px margin.
