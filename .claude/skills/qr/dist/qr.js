...
if (!text) {
  console.error(source_default.hex("#F87171")("Error: No text or URL provided."));
  process.exit(1);
}
try {
  const terminalQR = await import_qrcode.default.toString(text, {
    type: small ? "utf8" : "terminal",
    errorCorrectionLevel: "H",
    margin: 2
  });
  console.log("");
  console.log(source_default.bold.hex("#E8E6DF")("  QR Code Generator"));
  console.log(source_default.hex("#6A6860")(`  Content: ${text}`));
  console.log("");
  ...
  if (outputPath) {
    ...
    await import_qrcode.default.toFile(outputPath, text, {
      width: 200,
      margin: 2,
      color: { dark: "#000000", light: "#ffffff" },
      errorCorrectionLevel: "H"
    });
    console.log(source_default.hex("#4ADE80")(`  \u2713 Saved to ${outputPath}`));
    console.log("");
  }
} catch (err) {
  console.error(source_default.hex("#F87171")(`Error generating QR code: ${err instanceof Error ? err.message : err}`));
  process.exit(1);
}
