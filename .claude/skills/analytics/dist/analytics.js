...
function analyseRows(rows) {
  const active = rows.filter((r) => ["approved", "waitlist", "attended"].includes((r["approval_status"] || "").toLowerCase()));
  const expCol = "What is your experience level with Claude Code?";
  const sizeCol = "Number of employees at your company?";
  const topicsCol = "What Claude Code features or capabilities are you most interested in discussing at this meetup?";
  const expCounts = {};
  for (const r of active) {
    const v = (r[expCol] || "").trim();
    if (v)
      expCounts[v] = (expCounts[v] || 0) + 1;
  }
  const roleCounts = {};
  for (const r of active) {
  ...
  const topicCounts = topicDefs.map(({ label, keywords }) => ({
    label,
  ...
  const rolesSorted = Object.entries(roleCounts).sort((a, b) => b[1] - a[1]).map(([label, count]) => ({
    label,
    count,
...
function renderTopicChips(topics) {
  return topics.map((t, i) => {
    ...
    return source_default.bgHex(style.bg).hex(style.color)(` ${t.label} (${t.count}) `);
  }).join("  ");
}
function renderBarChart(items) {
  ...
  return items.map((item, i) => {
    ...
    return `${source_default.hex("#9A9890")(label)}  ${source_default.hex(colour)(bar)} ${source_default.hex("#6A6860")(pct)} ${source_default.hex("#5A5855")(count)}`;
  }).join(`
`);
}
function renderInsightCards(insights) {
  ...
  for (const insight of insights) {
    table.push([
      source_default.bold.hex("#E8E6DF")(String(insight.value).padStart(8)),
      source_default.hex("#6A6860")(insight.label)
    ]);
  }
  return table.toString();
}
function renderAnalyticsDashboard(data, filename) {
  const lines = [];
  lines.push("");
  lines.push(source_default.bold.hex("#E8E6DF")("  Attendee Analytics"));
  lines.push(source_default.hex("#5A5855")(`  ${filename} — ${data.total} registrants`));
  lines.push("");
...
if (!csvPath) {
  console.error(source_default.hex("#F87171")("Error: No CSV file path provided."));
  process.exit(1);
}
try {
  if (!existsSync(csvPath)) {
    console.error(source_default.hex("#F87171")(`Error: File not found: ${csvPath}`));
    process.exit(1);
  }
  const text = readFileSync(csvPath, "utf-8");
  const rows = parseLumaCSV(text);
  if (rows.length === 0) {
    console.error(source_default.hex("#F87171")("Error: CSV file is empty or could not be parsed."));
    process.exit(1);
  }
  ...
  if (Object.keys(data.expCounts).length > 0) {
    console.log(source_default.hex("#5A5855")("── " + source_default.bold.hex("#9A9890")("EXPERIENCE LEVELS") + " " + "─".repeat(40)));
    console.log("");
    for (const [level, count] of Object.entries(data.expCounts).sort((a, b) => b[1] - a[1])) {
      const pct = Math.round(count / data.total * 100);
      console.log(`  ${source_default.hex("#9A9890")(level.padEnd(35))} ${source_default.hex("#E8E6DF")(String(count).padStart(4))} ${source_default.hex("#6A6860")(`(${pct}%)`)}`);
    }
    console.log("");
  }
} catch (err) {
  console.error(source_default.hex("#F87171")(`Error: ${err instanceof Error ? err.message : "Could not parse CSV. Make sure it's a Luma guest export."}`));
  process.exit(1);
}
